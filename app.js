...
function acaiModifiers(qtd) {
  return [
    ...
    },
    {
      ...
      minSelect: qtd,
      maxSelect: qtd,
      description: `Escolha ${qtd} acompanhamentos`,
      options: [
...
const MENU = [
  ...
  { id:29, category:"tapioca", name:"Tapioca",
    ...
    modifiers:[recheioTapiocaModifier] },
  ...
  { id:74, category:"bebidas", name:"Coca-Cola Lata",
    ...
    modifiers:[{ name:"Opção", required:true, multiple:false,
      options:[{name:"Original",price:0},{name:"Zero",price:0}] }] },
...
// ============================================================
// RENDERIZAÇÃO DOS MODIFIERS (radio + checkbox)
// ============================================================
      ...
      <div class="modifier-group" id="modGroup-${mi}">
        <div class="modifier-title">
          ...
          ${mod.required
            ...
            : ""}
          ${isMultiple && mod.maxSelect
            ? `<span style="font-size:9px;color:var(--text-muted);font-weight:400"> — escolha ${mod.maxSelect}</span>`
            : ""}
        </div>
...
// Atualiza selectedModifiers para grupos RADIO
...
function syncCheckboxModifier(mi) {
  ...
  selectedModifiers[mi] = Array.from(checked).map(inp => {
  ...
  // Marca visual
  document.querySelectorAll(`#modalModifiers input[data-modifier-index="${mi}"]`).forEach(inp => {
...
function addToCartFromModal() {
  ...
  for (let mi = 0; mi < modalProduct.modifiers.length; mi++) {
    ...
    if (mod.multiple) {
      if (selected.length < (mod.minSelect || 1)) {
        showToast("Atenção ⚠️", `Escolha ${mod.minSelect} opções em: ${mod.name}`, "warn");
...
function openCheckout() {
  if (!cart.length) { showToast("Carrinho vazio 🛒","Adicione itens antes de finalizar","warn"); return; }
  if (isClosed()) return;
...
function validateStep1() {
  ...
  if (!name)  { markInputError("customerName");         showToast("Campo obrigatório ⚠️","Informe seu nome completo","warn"); return false; }
  if (phone.length < 10 || phone.length > 11) { markInputError("customerPhone"); showToast("Telefone inválido ⚠️","Informe um WhatsApp válido com DDD","warn"); return false; }
  if (!str)   { markInputError("customerStreet");       showToast("Endereço incompleto ⚠️","Informe o nome da rua","warn"); return false; }
  if (!num)   { markInputError("customerNumber");       showToast("Endereço incompleto ⚠️","Informe o número","warn"); return false; }
  if (!neigh) { markInputError("customerNeighborhood"); showToast("Endereço incompleto ⚠️","Informe o bairro","warn"); return false; }
...
function buildPixPayload(chave, nome, cidade, valor) {
  ...
  const payload =
    tlv("00","01") + tlv("01","12") + tlv("26",mai) +
    tlv("52","0000") + tlv("53","986") + tlv("54",valor.toFixed(2)) +
    tlv("58","BR") + tlv("59",clean(nome)) + tlv("60",clean(cidade)) +
    tlv("62",tlv("05","***")) + "6304";
...
function sendToWhatsApp() {
  ...
  if (pay === "dinheiro" && change === "yes" && changeAmt) msg += `  Troco para: ${changeAmt}\n`;
  if (pay === "pix") msg += `  _Valor PIX: ${fmt(total)}_\n`;
